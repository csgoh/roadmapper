...
class Roadmap:
    ...
    def __set_default_settings(self):
        ...
        if self.footer_text == "":
            self.footer_text = f"Generated by Roadmap Generator {self.VERSION}"
        if self.footer_font == "":
    ...
    def __draw_timeline(self, task_data):
        ...
        for x in range(self.timeline_item):
            timeline_x_pos = (
                ...
                + max_group_text_width
                + (self.__HSPACER * x)
                + 60  # Gap between Group Box and Task Box
            )
            ...
            if self.timeline_mode == self.WEEKLY:
                this_week = start_date + relativedelta(weeks=+x)
            ...
            if self.timeline_mode == self.MONTHLY:
                this_month = start_date + relativedelta(months=+x)
            ...
            if self.timeline_mode == self.QUARTERLY:
                this_month = start_date + relativedelta(months=+(x * 3))
            ...
            if self.timeline_mode == self.HALF_YEARLY:
                this_month = start_date + relativedelta(months=+(x * 6))
            ...
            if self.timeline_mode == self.YEARLY:
                this_month = start_date + relativedelta(months=+(x * 12))
                timeline_text = f"{this_month.year}"
                timeline_text_with_year = f"{this_month.year}"

            # print(f"{this_month=},{timeline_text},{timeline_text_with_year=}")
    ...
    def __draw_group(self, x, y, max_width, max_height, group_text):
    ...
    def __draw_groups(self, task_data, max_group_text_width, timeline_positions):
        ...
        for group_item in task_data.keys():
            ...
            if next_group_y_pos == 0:
                ...
                group_y_pos = next_group_y_pos + 25  # Gap between groups

            ## Set Task
            task_y_pos = group_y_pos

            milestone_in_group = False
            tasks = self.__roadmap_input[group_item].get("tasks", {})
            for task in tasks:
                for milestone in tasks[task].get("milestones", {}):
                    if milestone != {}:
                        milestone_in_group = True
                        break
            ...
            for task in self.__roadmap_input[group_item].get("tasks"):
                ...
                # Check whether task has milestones
                if milestone_in_group == False:
                    next_task_y_pos = (
                        task_y_pos + (text_height * j) + (self.__VSPACER * j)
                    )
                else:
                    next_task_y_pos = (
                        task_y_pos + (text_height * j) + (self.__VSPACER * j) + (20 * j)
                    )

                if j == 0:
                    group_y_start_pos = next_task_y_pos
